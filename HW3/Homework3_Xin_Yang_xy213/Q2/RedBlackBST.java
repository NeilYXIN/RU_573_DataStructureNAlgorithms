...
public class RedBlackBST<Key extends Comparable<Key>, Value> {
    ...
    ***************************************************************************/
    ...
    /**
     * Deep First Search to calculate the internal length
     *
     * @param node
     * @param currentDepth
     ...
     * @return
     */
    public double dfs(RedBlackBST.Node node, int currentDepth, double internalLength) {
    ...
    /**
     * Unit tests to show the average path length
     *
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int j = 1; j <= N; j++) {
            ...
            double pathLength = 0;
            if (st.root != null) {
                pathLength = st.dfs(st.root, 0, pathLength);
                System.out.println("Sorted Insertion: " +pathLength/N);
                sorted = pathLength/N;
            }
            ...
            for (int i = 0; i < N; i++) {
                //String key = StdIn.readString();
                st1.put(dataset.get(i),dataset.get(i));
            }
            double pathLength1 = 0;
            if (st1.root != null) {
                pathLength1 = st1.dfs(st1.root, 0, pathLength1);
                System.out.println("Random Insertion: " + pathLength1/N);
                random = pathLength1/N;
            }
            writer.write(  String.format("%.3f", log) + "," + String.format("%.3f", sorted)  + "," +  String.format("%.3f", random)  + "\n");
            writer.flush();
        }
    }
}
